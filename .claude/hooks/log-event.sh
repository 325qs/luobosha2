#!/usr/bin/env bash
#
# Yang.skills / meta-logging hook
#
# Passive event recorder. Writes one JSON line per event to
# .yang-cache/usage.jsonl in the project root. Never blocks (async fire-and-forget).
#
# Used by /yang-status to compute:
#   - "distance since last bump" (count of yang-predict invocations after last_bump_at)
...
# Output: appends one line to .yang-cache/usage.jsonl

set -uo pipefail

event_type="${1:-unknown}"
cache_dir="${CLAUDE_PROJECT_DIR:-.}/.yang-cache"
